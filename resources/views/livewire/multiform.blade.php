<div>
	@if($completed==false)
<div class="h-screen">
 
	<link rel="stylesheet" href="https://unpkg.com/tailwindcss@^1.0/dist/tailwind.min.css">
	<script src="https://cdn.jsdelivr.net/gh/alpinejs/alpine@v2.x.x/dist/alpine.js" defer></script>

	<style>
		[x-cloak] {
			display: none;
		}

		[type="checkbox"] {
			box-sizing: border-box;
			padding: 0;
		}

		.form-checkbox,
		.form-radio {
			-webkit-appearance: none;
			-moz-appearance: none;
			appearance: none;
			-webkit-print-color-adjust: exact;
			color-adjust: exact;
			display: inline-block;
			vertical-align: middle;
			background-origin: border-box;
			-webkit-user-select: none;
			-moz-user-select: none;
			-ms-user-select: none;
			user-select: none;
			flex-shrink: 0;
			color: currentColor;
			background-color: #fff;
			border-color: #e2e8f0;
			border-width: 1px;
			height: 1.4em;
			width: 1.4em;
		}

		.form-checkbox {
			border-radius: 0.25rem;
		}

		.form-radio {
			border-radius: 50%;
		}

		.form-checkbox:checked {
			background-image: url("data:image/svg+xml,%3csvg viewBox='0 0 16 16' fill='white' xmlns='http://www.w3.org/2000/svg'%3e%3cpath d='M5.707 7.293a1 1 0 0 0-1.414 1.414l2 2a1 1 0 0 0 1.414 0l4-4a1 1 0 0 0-1.414-1.414L7 8.586 5.707 7.293z'/%3e%3c/svg%3e");
			border-color: transparent;
			background-color: currentColor;
			background-size: 100% 100%;
			background-position: center;
			background-repeat: no-repeat;
		}
		
		.form-radio:checked {
			background-image: url("data:image/svg+xml,%3csvg viewBox='0 0 16 16' fill='white' xmlns='http://www.w3.org/2000/svg'%3e%3ccircle cx='8' cy='8' r='3'/%3e%3c/svg%3e");
			border-color: transparent;
			background-color: currentColor;
			background-size: 100% 100%;
			background-position: center;
			background-repeat: no-repeat;
		}
	</style>

	<div x-data="app()" x-cloak>
		<div class="max-w-3xl px-4 py-10 mx-auto">
			<div x-show.transition="step != 'complete'">	
				<!-- Top Navigation -->
				<div class="py-4 border-b-2">
					<div class="mb-1 text-xs font-bold leading-tight tracking-wide text-gray-500 uppercase" x-text="`Step: ${step} of 3`"></div>
					<div class="flex flex-col md:flex-row md:items-center md:justify-between">
						<div class="flex-1">
							<div x-show="step === 1">
								<div class="text-lg font-bold leading-tight text-gray-700">Personal Information</div>
							</div>
							
							<div x-show="step === 2">
								<div class="text-lg font-bold leading-tight text-gray-700">Document Checklist</div>
							</div>

							<div x-show="step === 3">
								<div class="text-lg font-bold leading-tight text-gray-700">Upload Documents</div>
							</div>
						</div>

						<div class="flex items-center md:w-64">
							<div class="w-full mr-2 bg-white rounded-full">
								<div class="h-2 text-xs leading-none text-center text-white bg-green-500 rounded-full" :style="'width: '+ parseInt(step / 3 * 100) +'%'"></div>
							</div>
							<div class="w-10 text-xs text-gray-600" x-text="parseInt(step / 3 * 100) +'%'"></div>
						</div>
					</div>
				</div>
				<!-- /Top Navigation -->

				<!-- Step Content -->
				<div class="py-10">	
					<div x-show.transition.in="step === 1">
						<div class="mb-5 text-center">
							<div class="relative w-32 h-32 mx-auto mb-4 bg-gray-100 border rounded-full shadow-inset">
								<img id="image" class="object-cover w-full h-32 rounded-full" :src="image" />
							</div>
							
							<label 
								for="fileInput"
								type="button"
								class="items-center justify-between px-4 py-2 font-medium text-left text-gray-600 bg-white border rounded-lg shadow-sm cursor-pointer inine-flex focus:outline-none hover:bg-gray-100"
							>
								<svg xmlns="http://www.w3.org/2000/svg" class="inline-flex flex-shrink-0 w-6 h-6 mr-1 -mt-1" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
									<rect x="0" y="0" width="24" height="24" stroke="none"></rect>
									<path d="M5 7h1a2 2 0 0 0 2 -2a1 1 0 0 1 1 -1h6a1 1 0 0 1 1 1a2 2 0 0 0 2 2h1a2 2 0 0 1 2 2v9a2 2 0 0 1 -2 2h-14a2 2 0 0 1 -2 -2v-9a2 2 0 0 1 2 -2" />
									<circle cx="12" cy="13" r="3" />
								</svg>						
								Passport Size Photo
							</label>
							
							<div class="w-48 mx-auto mt-1 text-xs text-center text-gray-500">Click to add passport size picture</div>
							<input autotfocus wire:model="pphoto" accept=".png,.jpg,.jpeg" id="fileInput" accept="image/*" class="hidden @error('pphoto')  border-red-500 @enderror" type="file" @change="let file = document.getElementById('fileInput').files[0]; 
								var reader = new FileReader();
								reader.onload = (e) => image = e.target.result;
								reader.readAsDataURL(file);">
								@error('pphoto')
								<p class="mt-4 text-xl italic text-red-500">
									{{ $message }}
								</p>
								@enderror
								
							</div>
						

						<div class="mb-5">
							<label for="bname" autofocus class="block mb-1 font-bold text-gray-700">Business Name</label>
							<input type="text" autofocus wire:model="bname"
								class="@error('bname')  border-red-500 @enderror w-full px-4 py-3 font-medium text-gray-600 rounded-lg shadow-sm  focus:outline-none focus:shadow-outline"
								placeholder="Enter your name...">
								@error('bname')
								<p class="mt-4 text-xl italic text-red-500">
									{{ $message }}
								</p>
								@enderror
							
							</div>

						<div class="mb-5">
							<label for="" class="block mb-1 font-bold text-gray-700">Business Type:</label>
							<select wire:model="bType" class="@error('bType')  border-red-500 @enderror w-full px-4 py-3 font-medium text-gray-600 rounded-lg shadow-sm  focus:outline-none focus:shadow-outline">
							<option value=''> Select Business Type</option>
							@forelse ($rates as $type )
								<option value="{{$type->id}}">{{$type->b_type}}</option>	
							@empty
								<option>***NOT AVAILABLE***</option>
							@endforelse
							</select>
								@error('bType')
                            <p class="mt-4 text-xl italic text-red-500">
                                {{ $message }}
                            </p>
                            @enderror
						</div>
						
						<div class="mb-5">
							<label for="name" autofocus class="block mb-1 font-bold text-gray-700">Name</label>
							<input type="text" autofocus wire:model="name"
								class="@error('name')  border-red-500 @enderror w-full px-4 py-3 font-medium text-gray-600 rounded-lg shadow-sm  focus:outline-none focus:shadow-outline"
								placeholder="Enter your name...">
								@error('name')
								<p class="mt-4 text-xl italic text-red-500">
									{{ $message }}
								</p>
								@enderror
							</div>
							
							<div class="mb-5">
								<label for="email"  class="block mb-1 font-bold text-gray-700">Email</label>
								<input type="email" wire:model="email"
									class="@error('email')  border-red-500 @enderror w-full px-4 py-3 font-medium text-gray-600 rounded-lg shadow-sm focus:outline-none focus:shadow-outline"
									placeholder="Enter your email address...">
								@error('email')
								<p class="mt-4 text-xl italic text-red-500">
									{{ $message }}
								</p>
								@enderror
							</div>
							<div class="mb-5">
								<label for="tel" class="block mb-1 font-bold text-gray-700">Phone Number</label>
								<input type="tel" wire:model="tel" pattern="[0-9]{3}-[0-9]{3}-[0-9]{4}"
									class="@error('tel')  border-red-500 @enderror w-full px-4 py-3 font-medium text-gray-600 rounded-lg shadow-sm  focus:outline-none focus:shadow-outline"
									placeholder="876-###-####">
							   @error('tel')
								<p class="mt-4 text-xl italic text-red-500">
									{{ $message }}
								</p>
								@enderror
							</div>
							
							
							
						<div class="mb-5">
							<label for="" class="block mb-1 font-bold text-gray-700">Upload your TRN:</label>
							<input type="file" accept=".png,.jpg,.jpeg" wire:model="trn" 
								class="@error('trn')  border-red-500 @enderror w-full px-4 py-3 font-medium text-gray-600 rounded-lg shadow-sm  focus:outline-none focus:shadow-outline">
						   @error('trn')
                            <p class="mt-4 text-xl italic text-red-500">
                                {{ $message }}
                            </p>
                            @enderror
						</div>
						
						
						<div class="mb-5">
							<label for="" class="block mb-1 font-bold text-gray-700">Upload your National Identification:</label>
							<input type="file" wire:model="nID" accept=".png,.jpg,.jpeg" 
								class="@error('nID')  border-red-500 @enderror w-full px-4 py-3 font-medium text-gray-600 rounded-lg shadow-sm  focus:outline-none focus:shadow-outline">
						   @error('nID')
                            <p class="mt-4 text-xl italic text-red-500">
                                {{ $message }}
                            </p>
                            @enderror
						</div>
							
						
							<div class="mb-5">
								<label for="" class="block mb-1 font-bold text-gray-700">Upload Business Proof of Address (Bill*):</label>
								<input type="file" wire:model="poa" accept=".pdf,.png,.jpg,.jpeg"
									class="@error('poa')  border-red-500 @enderror w-full px-4 py-3 font-medium text-gray-600 rounded-lg shadow-sm  focus:outline-none focus:shadow-outline">
							   @error('poa')
								<p class="mt-4 text-xl italic text-red-500">
									{{ $message }}
								</p>
								@enderror
							</div>
						
						<div class="mb-5">
							<label for="" class="block mb-1 font-bold text-gray-700">Loan Amount:</label>
							<input type="number" wire:model="loanAmt" 
								class="@error('loanAmt')  border-red-500 @enderror w-full px-4 py-3 font-medium text-gray-600 rounded-lg shadow-sm focus:outline-none focus:shadow-outline"
								placeholder="$">
						@error('loanAmt')
						<p class="mt-4 text-xl italic text-red-500">
							{{ $message }}
						</p>
						@enderror
					   </div>
						
						
						<div class="mb-5">
							<label for="" class="block mb-1 font-bold text-gray-700">Loan Period [year(s)]:</label>
							<input type="number" wire:model="period" 
								class="@error('period')  border-red-500 @enderror w-full px-4 py-3 font-medium text-gray-600 rounded-lg shadow-sm  focus:outline-none focus:shadow-outline"
								>
						 	@error('period')
                            <p class="mt-4 text-xl italic text-red-500">
                                {{ $message }}
                            </p>
                            @enderror
						</div>
						  

					</div>
					<div x-show.transition.in="step === 2">

						<div class="mb-5">
							<label for="top" class="block mb-1 font-bold text-gray-700">Please ensure to upload all documents listed below.</label>
							<div class="mt-2 mb-4 text-gray-600">
								

								<ul class="mt-2 ml-4 text-sm list-disc">
									<li>Loan Purpose Document</li>
									<li>Business Plan</li>
									<li>Balance sheet</li>
									<li>Bank Statement</li>
									<li>Business Registration Certifficate</li>
									<li>Business Income and Expenditure Statement</li>
									<li>Business Cashflow Statement Projections</li>
								</ul>	
							</div>

							<div class="relative">
								{{-- <input
									:type="togglePassword ? 'text' : 'password'"
									@keydown="checkPasswordStrength()"
									x-model="password"
									class="w-full px-4 py-3 font-medium text-gray-600 rounded-lg shadow-sm focus:outline-none focus:shadow-outline"
									placeholder="Your strong password..."> --}}

								{{-- <div class="absolute top-0 bottom-0 right-0 px-3 py-3 cursor-pointer" 
									@click="togglePassword = !togglePassword"
								>	
									<svg :class="{'hidden': !togglePassword, 'block': togglePassword }" xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-gray-500 fill-current" viewBox="0 0 24 24"><path d="M12 19c.946 0 1.81-.103 2.598-.281l-1.757-1.757C12.568 16.983 12.291 17 12 17c-5.351 0-7.424-3.846-7.926-5 .204-.47.674-1.381 1.508-2.297L4.184 8.305c-1.538 1.667-2.121 3.346-2.132 3.379-.069.205-.069.428 0 .633C2.073 12.383 4.367 19 12 19zM12 5c-1.837 0-3.346.396-4.604.981L3.707 2.293 2.293 3.707l18 18 1.414-1.414-3.319-3.319c2.614-1.951 3.547-4.615 3.561-4.657.069-.205.069-.428 0-.633C21.927 11.617 19.633 5 12 5zM16.972 15.558l-2.28-2.28C14.882 12.888 15 12.459 15 12c0-1.641-1.359-3-3-3-.459 0-.888.118-1.277.309L8.915 7.501C9.796 7.193 10.814 7 12 7c5.351 0 7.424 3.846 7.926 5C19.624 12.692 18.76 14.342 16.972 15.558z"/></svg>

									<svg :class="{'hidden': togglePassword, 'block': !togglePassword }" xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-gray-500 fill-current" viewBox="0 0 24 24"><path d="M12,9c-1.642,0-3,1.359-3,3c0,1.642,1.358,3,3,3c1.641,0,3-1.358,3-3C15,10.359,13.641,9,12,9z"/><path d="M12,5c-7.633,0-9.927,6.617-9.948,6.684L1.946,12l0.105,0.316C2.073,12.383,4.367,19,12,19s9.927-6.617,9.948-6.684 L22.054,12l-0.105-0.316C21.927,11.617,19.633,5,12,5z M12,17c-5.351,0-7.424-3.846-7.926-5C4.578,10.842,6.652,7,12,7 c5.351,0,7.424,3.846,7.926,5C19.422,13.158,17.348,17,12,17z"/></svg> --}}
								</div>
							</div>
							
							<div class="flex items-center h-3 mt-4">
								{{-- <div class="flex justify-between w-2/3 h-2">	
									<div :class="{ 'bg-red-400': passwordStrengthText == 'Too weak' ||  passwordStrengthText == 'Could be stronger' || passwordStrengthText == 'Strong password' }" class="w-1/3 h-2 mr-1 bg-gray-300 rounded-full"></div>
									<div :class="{ 'bg-orange-400': passwordStrengthText == 'Could be stronger' || passwordStrengthText == 'Strong password' }" class="w-1/3 h-2 mr-1 bg-gray-300 rounded-full"></div>
									<div :class="{ 'bg-green-400': passwordStrengthText == 'Strong password' }" class="w-1/3 h-2 bg-gray-300 rounded-full"></div>
								</div> --}}
								<div x-text="passwordStrengthText" class="ml-3 text-sm font-medium leading-none text-gray-500"></div>
							</div>

							<p class="mt-5 text-gray-600"></p>
						</div>

					</div>
					<div x-show.transition.in="step === 3">
					
						<div class="flex">
						
							<div class="mb-5">
								<label for="" class="block mb-1 font-bold text-gray-700">Loan Purpose Document:</label>
								<input type="file" wire:model="loanPDoc" accept=".pdf,.docs,.docx"
									class="@error('loanPDoc')  border-red-500 @enderror w-full px-4 py-3 font-medium text-gray-600 rounded-lg shadow-sm  focus:outline-none focus:shadow-outline">
							   @error('loanPDoc')
								<p class="mt-4 text-xl italic text-red-500">
									{{ $message }}
								</p>
								@enderror
							</div>
						</div>
						
						<div class="flex">
							<div class="mb-5">
								<label for="" class="block mb-1 font-bold text-gray-700">Business Plan:</label>
								<input type="file" wire:model="bPlan" accept=".pdf,.docs,.docx"
									class="@error('bPlan')  border-red-500 @enderror w-full px-4 py-3 font-medium text-gray-600 rounded-lg shadow-sm  focus:outline-none focus:shadow-outline">
							   @error('bPlan')
								<p class="mt-4 text-xl italic text-red-500">
									{{ $message }}
								</p>
								@enderror
							</div>
						</div>
						
						<div class="flex">
							<div class="mb-5">
								<label for="" class="block mb-1 font-bold text-gray-700">Balance Sheet:</label>
								<input type="file" wire:model="bSheet" accept=".pdf,.docs,.docx"
									class="@error('bSheet')  border-red-500 @enderror w-full px-4 py-3 font-medium text-gray-600 rounded-lg shadow-sm  focus:outline-none focus:shadow-outline">
							   @error('bSheet')
								<p class="mt-4 text-xl italic text-red-500">
									{{ $message }}
								</p>
								@enderror
							</div>
						</div>
						
						<div class="flex">
							<div class="mb-5">
								<label for="" class="block mb-1 font-bold text-gray-700">Bank Book Statement:</label>
								<input type="file" wire:model="bStatement" accept=".pdf,.docs,.docx"
									class="@error('bStatement')  border-red-500 @enderror w-full px-4 py-3 font-medium text-gray-600 rounded-lg shadow-sm  focus:outline-none focus:shadow-outline">
							   @error('bStatement')
								<p class="mt-4 text-xl italic text-red-500">
									{{ $message }}
								</p>
								@enderror
							</div>
						</div>

						<div class="flex">
							<div class="mb-5">
								<label for="" class="block mb-1 font-bold text-gray-700">Business Registration Certificate:</label>
								<input type="file" wire:model="bRCert" accept=".pdf,.docs,.docx"
									class="@error('bRCert')  border-red-500 @enderror w-full px-4 py-3 font-medium text-gray-600 rounded-lg shadow-sm  focus:outline-none focus:shadow-outline">
							   @error('bRCert')
								<p class="mt-4 text-xl italic text-red-500">
									{{ $message }}
								</p>
								@enderror
							</div>
						</div>
						
						<div class="flex">
							<div class="mb-5">
								<label for="" class="block mb-1 font-bold text-gray-700">Business Income and Expenditure Statement:</label>
								<input type="file" wire:model="bIEStatement" accept=".pdf,.docs,.docx"
									class="@error('bIEStatement')  border-red-500 @enderror w-full px-4 py-3 font-medium text-gray-600 rounded-lg shadow-sm  focus:outline-none focus:shadow-outline">
							   @error('bIEStatement')
								<p class="mt-4 text-xl italic text-red-500">
									{{ $message }}
								</p>
								@enderror
							</div>
						</div>
						
						<div class="flex">
							<div class="mb-5">
								<label for="" class="block mb-1 font-bold text-gray-700">Business Cashflow Statement and Projections:</label>
								<input type="file" wire:model="bCFStatement" accept=".pdf,.docs,.docx"
									class="@error('bCFStatement')  border-red-500 @enderror w-full px-4 py-3 font-medium text-gray-600 rounded-lg shadow-sm  focus:outline-none focus:shadow-outline">
							   @error('bCFStatement')
								<p class="text-xl italic text-red-500 ">
									{{ $message }}
								</p>
								@enderror
							</div>
						</div>
					</div>
					</div>
					<!-- Bottom Navigation -->	
		<div class="py-5 bg-orange-400 shadow-md " x-show="step != 'complete'">
			<div class="max-w-3xl px-4 mx-auto">
				<div class="flex justify-between">
					<div class="w-1/2">
						<button
							x-show="step > 1"
							@click="step--"
							class="w-32 px-5 py-2 font-medium text-center text-gray-600 bg-white border rounded-lg shadow-sm focus:outline-none hover:bg-gray-100" 
						>Previous</button>
					</div>

					<div class="w-1/2 text-right">
						<button type="submit"
						wire:click.prevent="loanapplicationsubmission"
							x-show="step < 3"
							@click="step++"
							class="w-32 px-5 py-2 font-medium text-center text-white bg-blue-500 border border-transparent rounded-lg shadow-sm focus:outline-none hover:bg-blue-600" 
						>Next</button>

						<button
						type="submit"
						wire:click.prevent='loanapplicationsubmission'
							@click="step = 3"
							x-show="step === 3"
							class="w-32 px-5 py-2 font-medium text-center text-white bg-blue-500 border border-transparent rounded-lg shadow-sm focus:outline-none hover:bg-blue-600" 
						>Complete</button>
					</div>
				</div>
			</div>
		</div>
		</div>
		@elseif($completed==true)
		<div class="max-w-3xl px-4 py-10 mx-auto">
			<div class="flex items-center justify-between p-10 bg-white rounded-lg shadow">
				<div>
					<svg class="w-20 h-20 mx-auto mb-4 text-green-500" viewBox="0 0 20 20" fill="currentColor">  <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/></svg>

					<h2 class="mb-4 text-2xl font-bold text-center text-gray-800">Loan Application Success</h2>

					<div class="mb-8 text-gray-600">
						Thank you. Your application is now in processing. An email will be sent to you for an interview.
					</div>

					<button
						type="submit"
						wire:click.prevent="home"
						class="block w-40 px-5 py-2 mx-auto font-medium text-center text-gray-600 bg-white border rounded-lg shadow-sm focus:outline-none hover:bg-gray-100" 
					>Back to home</button>
				</div>
			</div>
		</div>
		@endif
		<!-- / Step Content -->
		</div>
	</div>	
</div>

	<script>
		function app() {
			return {
				step: 1, 
				passwordStrengthText: '',
				togglePassword: false,

				image: 'data:image/jpeg;base64,',
				password: '',
				gender: 'Male',

				checkPasswordStrength() {
					var strongRegex = new RegExp("^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#\$%\^&\*])(?=.{8,})");
					var mediumRegex = new RegExp("^(((?=.*[a-z])(?=.*[A-Z]))|((?=.*[a-z])(?=.*[0-9]))|((?=.*[A-Z])(?=.*[0-9])))(?=.{6,})");

					let value = this.password;

					if (strongRegex.test(value)) {
						this.passwordStrengthText = "Strong password";
					} else if(mediumRegex.test(value)) {
						this.passwordStrengthText = "Could be stronger";
					} else {
						this.passwordStrengthText = "Too weak";
					}
				}
			}
		}
	</script>
</div>
</div>
